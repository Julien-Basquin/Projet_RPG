...
public class NodeEquipement extends Node {

	public NodeEquipement(float x, float y) {
		id = globalId++;
	...
	public NodeEquipement(NodeCategorieEnum category, float x, float y) {
		id = globalId++;
	...
	public NodeEquipement(Node node) {
	...
	public void redo(EventsEnum event) {
		...
		case ADD:
			...
			MainWindow.previousStates.push(new ObjectEvent(new NodeEquipement(this), event + "_Node", objectEvent.getGroupId()));
			break;
		case DELETE:
			node.remove();
		...
		case EDIT:
			...
			break;
		case MOVE:
			MainWindow.previousStates.push(new ObjectEvent(new NodeEquipement(node), event + "_Node", objectEvent.getGroupId()));
	...
	public void undo(EventsEnum event) {
		...
		case ADD:	//Utilisation du noeud récupéré car différent de this
			...
			MainWindow.nextStates.push(new ObjectEvent(new NodeEquipement(node), event + "_Node", objectEvent.getGroupId()));
			break;
		case DELETE:	//Utilsiation de this car node n'existe plus
			//Ajout de this au graphe
			graph.addNode(this, true);

			MainWindow.nextStates.push(new ObjectEvent(new NodeEquipement(this), event + "_Node", objectEvent.getGroupId()));
			break;
		case EDIT:
			//TODO Edition d'un noeud
			break;
