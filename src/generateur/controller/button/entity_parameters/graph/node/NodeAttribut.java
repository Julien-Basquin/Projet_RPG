...
public class NodeAttribut extends Node {
	...
	public void redo(EventsEnum event) {
		...
		case ADD:
			...
			Generator.previousStates.push(new ObjectEvent(new NodeAttribut(this), event + "_Node", objectEvent.getGroupId()));
			break;
		case DELETE:
			node.remove();
		...
		case EDIT:
			...
			break;
		case MOVE:
			Generator.previousStates.push(new ObjectEvent(new NodeAttribut(node), event + "_Node", objectEvent.getGroupId()));
	...
	public void undo(EventsEnum event) {
		...
		case ADD:	//Utilisation du noeud récupéré car différent de this
			...
			Generator.nextStates.push(new ObjectEvent(new NodeAttribut(node), event + "_Node", objectEvent.getGroupId()));
			break;
		case DELETE:	//Utilsiation de this car node n'existe plus
			//Ajout de this au graphe
			graph.addNode(this, true);

			Generator.nextStates.push(new ObjectEvent(new NodeAttribut(this), event + "_Node", objectEvent.getGroupId()));
			break;
		case EDIT:
			//TODO Edition d'un noeud
			break;
